...
class DPN(nn.Module):
    def __init__(self, cfg, in_channels=3):
        ...
        self.max_pooling_2x1 = nn.MaxPool2d((2, 1))
    ...
    def forward(self, x):
        out = self.max_pooling_2x1(F.relu(self.bn1(self.conv1(x))))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
def DPN92(in_channels=3):
    ...
    return DPN(cfg, in_channels=in_channels)


if __name__ == "__main__":
    net = DPN92()
    x = torch.randn(10,3,32,400)
    y = net(x)
    print(y.shape)
